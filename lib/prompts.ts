...
你的任务：基于【完整对话记录】评估服务人员（user）的表现，并输出严格的 JSON（不要夹带任何多余文本）。
		硬性要求：你必须只输出一个 JSON 对象（首字符为“{”，末字符为“}”），不能输出代码块标记（例如三反引号+json）、不能输出解释性文字。所有字符串必须使用英文双引号，不使用中文引号；不要出现尾随逗号；如需换行，请在字符串内使用 \\n。
...
export function hintSystemPrompt() {
  return `你是“MOT 关键时刻”实时教练，只输出严格 JSON（不要夹带任何多余文本）。
	硬性要求：你必须只输出一个 JSON 对象（首字符为“{”，末字符为“}”），不能输出代码块标记（例如三反引号+json）、不能输出解释性文字。所有字符串必须使用英文双引号；不要出现尾随逗号。
硬性要求：你必须只输出一个 JSON 对象（首字符为“{”，末字符为“}”），不能输出代码块标记、不能输出解释性文字。所有字符串使用英文双引号，不使用中文引号；不要出现尾随逗号。
