...
硬性要求：你必须只输出一个 JSON 对象（首字符为“{”，末字符为“}”），不能输出代码块标记（例如```json）、不能输出解释性文字。所有字符串必须使用英文双引号，不使用中文引号；不要出现尾随逗号；如需换行，请在字符串内使用 \n。
...
硬性要求：你必须只输出一个 JSON 对象（首字符为“{”，末字符为“}”），不能输出代码块标记（例如```json）、不能输出解释性文字。所有字符串必须使用英文双引号；不要出现尾随逗号。
